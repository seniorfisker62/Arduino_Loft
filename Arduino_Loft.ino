...
void ReceiveMessage()
{
	...
	if(packetSize)
	{
		...
		if(theVal.substring(0,1) == "5") // temperature sensor 1
		{     		
			//dtostrf(temperature[Sensor1], 6, 2, tempstr);	
			strcpy(tempstr, "LO T1");			
		}
		else if(theVal.substring(0,1) == "6") // humidity sensor 1
		{		
			//dtostrf(humidity[Sensor1], 6, 2, tempstr);
			strcpy(tempstr, "LO H1");			
		}    
		else if(theVal.substring(0,1) == "7") // temperature sensor 2
		{		
			//dtostrf(temperature[Sensor2], 6, 2, tempstr);	
			strcpy(tempstr, "LO T2");			
		}
		else if(theVal.substring(0,1) == "8") // humidity sensor 2
		{
			//dtostrf(humidity[Sensor2], 6, 2, tempstr);
			strcpy(tempstr, "LO H2");
		}		
		else
		{				
